...
test("10-41 historial no aplica pruning automático y expone pruning explícito", async()=>{const a=new TradeAuditService({storage:new MemoryStorage()});for(const id of ["1","2","3"]){const s=auditFixture();s.id=id;await a.persistTerminal(s);}assert.deepEqual((await a.getHistory()).map(x=>x.sessionId),["3","2","1"]);assert.deepEqual(await a.prune({maxCount:2}),{before:3,after:2,removed:1});assert.equal((await a.getHistory()).length,3);});
